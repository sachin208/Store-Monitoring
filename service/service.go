...
func (s *Service) appendBusinessHoursTableDataInMap() {
	...
	for _, businessHoursData := range businessHoursTableData {
		...
		if len(storeIdToAllTableDataMap[businessHoursData.StoreId].WeekDaysWithStartAndEndTimeList) == 0 {
			storeIdToAllTableDataMap[businessHoursData.StoreId] = dto.AllTableDataDto{WeekDaysWithStartAndEndTimeList: []dto.WeekDaysWithStartAndEndTime{weekDaysWithStartAndEndTimeDto}}
		} else {
...
func (s *Service) performBusinessLogicOfCalculatingUpAndDownTime() map[string]dto.UpAndDownTimeResponseDto {
	...
	for storeId, data := range storeIdToAllTableDataMap {
		...
		var totalBusinessMinutesInHour = 0
		...
		for _, weekDay := range data.WeekDaysWithStartAndEndTimeList {
			...
			if strconv.Itoa(weekDay.DayOfWeek) == currentTime.Weekday().String() {
			...
			if totalBusinessMinutesInHour == 0 && weekDayWithStartTimeMap[weekDay.DayOfWeek].Hour() == currentTime.Hour() && weekDayWithEndTimeMap[weekDay.DayOfWeek].Hour() == currentTime.Hour() {
				totalBusinessMinutesInHour += int(timeDifference(currentTime, weekDayWithStartTimeMap[weekDay.DayOfWeek]).Minutes())
				totalBusinessMinutesInHour += int(timeDifference(currentTime, weekDayWithEndTimeMap[weekDay.DayOfWeek]).Minutes())
			}
		}
		for _, statusAndTimeData := range data.StatusWithTimeList {
			currentDay := int(statusAndTimeData.TimestampUtc.Weekday())
			weekdayWithStatusMap[currentDay] = append(weekdayWithStatusMap[currentDay], statusAndTimeData)
		}
		...
		for weekdayNumber, weekdayWithStatusList := range weekdayWithStatusMap {
			...
			for _, weekdayWithStatus := range weekdayWithStatusList {
				if weekdayWithStatus.TimestampUtc.Before(weekDayWithStartTimeMap[weekdayNumber]) == true {
			...
			statusWithTimeInActiveBusinessHoursList = []dto.StatusWithTimeDto{{Status: "active", TimestampUtc: weekDayWithEndTimeMap[weekdayNumber]}}
		}
		...
		lastUpAndDownTimeResponse := dto.UpAndDownTimeResponseDto{
			...
			UpTimeLastWeek:   strconv.Itoa(totalBusinessHoursInWeek - lastDownTimeForWeek),
			DownTimeLastDay:  strconv.Itoa(lastDownTimeForDay),
			UpTimeLastDay:    strconv.Itoa(totalBusinessHoursInDay - lastDownTimeForDay),
			DownTimeLastHour: strconv.Itoa(lastDownTimeForHour),
			UpTimeLastHour:   strconv.Itoa(totalBusinessMinutesInHour - lastDownTimeForHour),
		}
...
func getUtcTime(currentTime string, currentTimezone string) time.Time {
	...
	t, err := time.ParseInLocation(longForm, currentTime, location)
	if err != nil {
		fmt.Println(err)
		return time.Time{}
	}
